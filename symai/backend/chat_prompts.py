...
PROMPTS_DIR = Path(__file__).parent / "engines" / "neurosymbolic" / "prompts"
...
def render_chat_system_prompt(argument) -> str:
    ...
    if prop.response_format:
        try:
            ...
            msg = (
                "Response format type is required! Expected format "
                '`{"type": "json_object"}` or another supported type.'
            )
