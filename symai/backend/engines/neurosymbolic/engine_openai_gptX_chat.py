...
class GPTXChatEngine(Engine, OpenAIMixin):
    ...
    def __init__(
        ...
        *,
        ...
        try:
            # NOTE: When caller supplies client_timeout / client_max_retries, override
            ...
            self.client = openai.Client(**self._build_client_kwargs({"api_key": openai.api_key}))
    ...
    def _resolve_token_config(self) -> tuple[int, int]:
        ...
        if self.model == "gpt-4":
            ...
            return 3, 1
        msg = f"num_tokens_from_messages() is not implemented for model {self.model}. See https://cookbook.openai.com/examples/how_to_count_tokens_with_tiktoken for information on how messages are converted to tokens."
    ...
    def _should_skip_truncation(self, prompts: list[dict]) -> bool:
        if len(prompts) != 2 and all(prompt["role"] in ["system", "user"] for prompt in prompts):
            logger.warning(
                "Token truncation currently supports only two messages, from 'user' and 'system' (got %s). Returning original prompts.",
                len(prompts),
            )
            return True
        return False
    ...
    def _collect_user_tokens(
        ...
        if isinstance(content, list):
            ...
            return user_tokens, None
        msg = f"Unknown content type: {type(content)}. Format input according to the documentation. See https://platform.openai.com/docs/api-reference/chat/create?lang=python"
                ...
                if img_.startswith("http") or img_.startswith("data:image"):
                    ...
                    if img_.startswith("frames:"):
                        ...
                        if max_used_frames < 1 or max_used_frames > max_frames_spacing:
                            msg = f"Invalid max_used_frames value: {max_used_frames}. Expected value between 1 and {max_frames_spacing}"
                            raise ValueError(msg)
    ...
    def truncate(
        ...
        logger.warning(
            "Executing %s truncation to fit within %s tokens. "
            "Combined prompts (%s tokens) exceed maximum allowed tokens "
            "of %s (%.1f%% of context). "
            "You can control this behavior by setting 'truncation_percentage' (current: %.2f) "
            "and 'truncation_type' (current: '%s') parameters. "
            "Set 'truncation_percentage=1.0' to deactivate truncation (will fail if exceeding context window). "
            "Choose 'truncation_type' as 'head' to keep the end of prompts or 'tail' to keep the beginning.",
            truncation_type,
            max_prompt_tokens,
            total_tokens,
            max_prompt_tokens,
    ...
    def _prepare_request_payload(self, messages, argument):
        ...
        if max_tokens is not None:
            ...
            if max_tokens > self.max_response_tokens:
                logger.warning(
                    "Provided 'max_tokens' (%s) exceeds max response tokens (%s). "
                    "Truncating to %s to avoid API failure.",
                    max_tokens,
        ...
        if max_completion_tokens is not None and max_completion_tokens > self.max_response_tokens:
            logger.warning(
                "Provided 'max_completion_tokens' (%s) exceeds max response tokens (%s). "
                "Truncating to %s to avoid API failure.",
