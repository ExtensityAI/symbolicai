...
from symai.backend.engines.neurosymbolic.orcarouter.models import (
    SUPPORTED_ORCAROUTER_MODELS,
    OrcaRouterOptions,
    OrcaRouterPayload,
    OrcaRouterRequest,
    OrcaRouterResponse,
    orcarouter_model_spec_for,
    orcarouter_strip_prefix,
)
from symai.backend.engines.neurosymbolic.orcarouter.stream import OrcaRouterStreamAdapter
...
ORCAROUTER_CHAT_COMPLETIONS_URL = "https://api.orcarouter.ai/v1/chat/completions"


class OrcaRouterEngine(Engine):
    def __init__(
        self,
    ...
    def id(self) -> str:
        if self.model in SUPPORTED_ORCAROUTER_MODELS and self.api_key:
            return "neurosymbolic"
        return super().id()  # default to unregistered

    def api_max_context_tokens(self) -> int:
        return orcarouter_model_spec_for(self.model).context_tokens

    def api_max_response_tokens(self) -> int:
        return orcarouter_model_spec_for(self.model).response_tokens

    def compute_required_tokens(self, _messages: list[dict]) -> int:
        msg = 'Method "compute_required_tokens" not implemented for OrcaRouterEngine.'
        raise NotImplementedError(msg)

    def compute_remaining_tokens(self, _prompts: list[dict]) -> int:
        msg = 'Method "compute_remaining_tokens" not implemented for OrcaRouterEngine.'
        raise NotImplementedError(msg)
    ...
    def build_request(self, argument) -> OrcaRouterRequest:
        allowed_request_kwargs = (
            set(OrcaRouterPayload.model_fields)
            | set(OrcaRouterOptions.model_fields)
            | {"max_completion_tokens"}
        )
        payload_kwargs = self.collect_request_kwargs(argument, allowed_request_kwargs)
        option_kwargs = {
            key: payload_kwargs.pop(key)
            for key in OrcaRouterOptions.model_fields
            if key in payload_kwargs
        }

        payload_kwargs["model"] = orcarouter_strip_prefix(payload_kwargs.get("model", self.model))
        orcarouter_model_spec_for(payload_kwargs["model"])
        payload_kwargs["messages"] = strip_cache_breakpoints_from_messages(
            argument.prop.prepared_input
        )
        # NOTE: OrcaRouter's canonical field is max_tokens; accept max_completion_tokens
        ...
        # truncates generation at the first character. OrcaRouter omits stop when unset.
        if not payload_kwargs.get("stop"):
            payload_kwargs["stop"] = None

        if payload_kwargs.get("stream"):
            # NOTE: usage is required on OrcaRouterResponse (MetadataTracker reads it),
        ...
        payload = OrcaRouterPayload.model_validate(payload_kwargs)
        options = OrcaRouterOptions.model_validate(option_kwargs)
    ...
    def forward(self, argument):
        if self.id() != "neurosymbolic":
            msg = (
                "OrcaRouter engine is not configured. Please set a supported "
    ...
    def call_request(self, request: OrcaRouterRequest):
        max_retries = (
        ...
        return OrcaRouterResponse.model_validate(response.json())

    def parse_response(self, response: OrcaRouterResponse):
    ...
    def _apply_self_prompt(self, argument, system, user_prompt):
        ...
        if prop.instance._kwargs.get("self_prompt", False) or prop.self_prompt:
            ...
            if res is None:
                msg = "Self-prompting failed for OrcaRouterEngine."
                raise ValueError(msg)
    ...
    def _collect_stream_response(self, request: OrcaRouterRequest, max_retries: int):
        adapter = OrcaRouterStreamAdapter()
        ...
        return OrcaRouterResponse.model_validate(
            {
                "choices": [
                    {
                        "index": 0,
    ...
    def _prepare_raw_input(self, argument):
        ...
        if not value:
            msg = "A prompt instruction is required for OrcaRouterEngine when raw_input is enabled."
            raise ValueError(msg)
    ...
    def _process_function_calls(self, response: OrcaRouterResponse, metadata: dict) -> dict:
