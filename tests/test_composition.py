...
class TestComposition(unittest.TestCase):
    ...
    def test_result_clean(self):
        expr = Expression()
        sym = expr.fetch(url='https://community.dynatrace.com/t5/Dynatrace-Open-Q-A/Dynatrace-Managed-will-support-strategy-for-RHEL-8-5-8-7-and-8-9/td-p/179194')
    ...
    def test_filter(self):
        ...
        self.assertTrue(res == 'Sports, Music, Art, Theater, Writing')
    
    def test_modify(self):
        sym = Symbol('Mathematic, Physics, Sports, Music, Art, Theater, Writing')
        res = sym.modify('to lower case')
        self.assertTrue(res == 'mathematic, physics, sports, music, art, theater, writing')
    ...
    def test_execute(self):
        sym = Symbol("""
from botdyn import Symbol
def test_inception():
    sym = Symbol('Hello World')
    res = sym.translate('Spanish')
    return 'Hola Mundo' == res.value, res
val, res = test_inception()
test = 'it works'
""")    
        res = sym.execute()
        self.assertTrue('test' in res)
        self.assertTrue(res['test'] == 'it works')
        self.assertTrue('val' in res)
        self.assertTrue(type(res['val']) == bool)
        self.assertTrue('res' in res)
        self.assertTrue(res['res'] == 'Hola Mundo')
        self.assertTrue(res['val'])
        
    def test_execute_pseudo_code(self):
        expr = Expression()
        code = expr.open('examples/pseudo_code.txt')
    ...
    def test_list(self):
        sym = Symbol("""
modified:   botdyn/backend/driver/webclient.py
modified:   botdyn/backend/engine_gpt3.py
modified:   botdyn/backend/engine_userinput.py
modified:   botdyn/core.py
modified:   botdyn/expressions.py
modified:   botdyn/functional.py
modified:   botdyn/pre_processors.py
modified:   botdyn/prompts.py
...
""")
    ...
    def test_dql(self):
        sym = DQL()
        res = sym('Query all logs and show the difference between fields and fieldsAdd. While the fields command defines the result table by the fields specified, the fieldsAdd command adds new fields to the existing fields.')
        self.assertTrue('fetch' in res, res)
        res = res.convert('SQL')
        self.assertTrue('FROM' in res, res)
    ...
    def test_setitem(self):
        sym = Symbol('Dynatrace is a cool company.')
        sym['adjective'] = 'smart'
    ...
    def test_cluster_component(self):
        ...
        sym = Symbol([c for c in stream(expr('examples/paper.pdf', n_pages=1))])
        cluster = Cluster()
        res = cluster(sym)
        mapper = Map()
        res = mapper(res)
