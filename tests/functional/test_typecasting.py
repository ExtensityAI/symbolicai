...
@pytest.mark.parametrize(
    "rsp, return_constraint, expected_output",
    [
        # String tests
        ("test", str, "test"),
        (42, str, "42"),
...
@pytest.mark.parametrize(
    "invalid_input, return_constraint",
    [
        ("not a list", list),
        ("not a tuple", tuple),
        ("not a set", set),
...
def test_cast_return_type_invalid_input(invalid_input, return_constraint):
    with pytest.warns(UserWarning):
        result = _cast_return_type(
            invalid_input, return_constraint, ProbabilisticBooleanMode.TOLERANT
        )
    assert result == invalid_input


@pytest.mark.parametrize(
    "empty_input, return_constraint, expected",
    [
        ("", bool, False),
        ([], bool, False),
        ({}, bool, False),
