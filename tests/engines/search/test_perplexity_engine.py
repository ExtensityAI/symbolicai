...
try:
    ...
    pytestmark.append(
        pytest.mark.skipif(True, reason=f"Perplexity interface initialization failed: {e}")
    )
...
def test_perplexity_basic_query(perplexity_interface):
    ...
    assert isinstance(res, PerplexitySearchResult), (
        "Response should be a PerplexitySearchResult instance."
    )
...
def test_perplexity_max_tokens(perplexity_interface):
    ...
    assert len(res._value.split()) < max_tokens_val * 3, (
        f"Response seems too long for max_tokens={max_tokens_val}"
    )
...
def test_perplexity_return_images_parameter_handling(perplexity_interface):
    ...
    assert isinstance(res_with_images, PerplexitySearchResult)
    ...
    assert isinstance(res_without_images, PerplexitySearchResult)
...
def test_perplexity_return_related_questions_parameter_handling(perplexity_interface):
    ...
    assert isinstance(res_with_related, PerplexitySearchResult)
    ...
    assert isinstance(res_without_related, PerplexitySearchResult)
...
def test_perplexity_presence_penalty(perplexity_interface):
    ...
    query = (
        "Summarize the plot of 'Hamlet' ensuring to cover diverse aspects without dwelling on one."
    )
...
def test_perplexity_response_format_json(perplexity_interface):
    ...
    try:
        ...
        assert isinstance(json_output["num_seasons_in_nba"], int), (
            "num_seasons_in_nba should be an integer"
        )
...
def test_perplexity_response_format_regex(perplexity_interface):
    ...
    response_format = {"type": "regex", "regex": {"regex": ipv4_regex}}

    res = perplexity_interface(
        query, system_message=system_message, response_format=response_format
    )
    ...
    assert ipv4_pattern.fullmatch(res._value.strip()), (
        f"Response '{res._value}' does not match IPv4 pattern"
    )
