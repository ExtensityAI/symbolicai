...
from symai.backend.provider_engines import Capability, create_provider_engine_handle
...
def test_dynamic_provider_engine_releases_its_handle_on_exit():
...
def test_dynamic_provider_engine_uses_explicit_transport_options(monkeypatch):
    captured = {}
    create_engine_handle = provider_engines.create_provider_engine_handle

    def capture_options(**kwargs):
        captured["options"] = kwargs["options"]
        return create_engine_handle(**kwargs)

    monkeypatch.setattr(provider_engines, "create_provider_engine_handle", capture_options)
    ...
    assert captured["options"] == provider_engines.ProviderTransportOptions(
        request_timeout=42.0,
        connect_timeout=3.0,
        connect_retries=2,
    )
...
def test_repository_replacement_closes_previous_provider_handle(monkeypatch):
    monkeypatch.setitem(
        SYMAI_CONFIG,
