...
def test_new_models_are_registered():
    assert "claude-opus-4-8" in anthropic_mixin.SUPPORTED_REASONING_MODELS
...
def test_claude_opus_4_7_and_4_8_use_reasoning_engine():
...
def test_claude_opus_4_7_and_4_8_without_thinking_omit_sampling_params():
    ...
    for model in ("claude-opus-4-8", "claude-opus-4-7"):
        with patch.object(mod.anthropic, "Anthropic"):
            engine = mod.ClaudeXReasoningEngine(api_key=DUMMY_KEY, model=model)
...
def test_claude_opus_4_7_and_4_8_thinking_is_adaptive():
    from symai.backend.engines.neurosymbolic import engine_anthropic_claudeX_reasoning as mod
...
def test_claude_opus_4_8_long_context_does_not_add_beta_header():
    from symai.backend.engines.neurosymbolic import engine_anthropic_claudeX_reasoning as mod

    with patch.object(mod.anthropic, "Anthropic"):
        engine = mod.ClaudeXReasoningEngine(api_key=DUMMY_KEY, model="claude-opus-4-8")

    argument = SimpleNamespace(
        kwargs={"long_context_1m": True},
        prop=SimpleNamespace(response_format=None),
    )
    payload = engine._prepare_request_payload(argument)
