...
from symai.backend.engines.neurosymbolic.orcarouter.engine import (
...
class TestOrcaRouterEngine(NeurosymbolicEngineTestInterface):
    engine_cls = OrcaRouterEngine
    supported_models = tuple(SUPPORTED_ORCAROUTER_MODELS)
    model_specs = ORCAROUTER_MODEL_SPECS
    default_model = "orcarouter:orcarouter/auto"
    response_cls = OrcaRouterResponse
    wire_provider = "orcarouter"
    wire_operation = "chat.completions.create"
    wire_url = ORCAROUTER_CHAT_COMPLETIONS_URL
    supports_streaming = True
    api_pinned = API_PINNED

    def spec_for(self, model):
        return self.model_specs[orcarouter_strip_prefix(model)]

    def expected_wire_model(self, model=None):
        return orcarouter_strip_prefix(model or self.default_model)
    ...
    def test_forward_streams_sse_and_aggregates_response(self):
        ...
        assert isinstance(raw_output, OrcaRouterResponse)
    ...
    def test_build_request_strips_provider_prefix_from_wire_model(self):
        ...
        assert request.body()["model"] == "orcarouter/auto"
