...
def assert_image_file_created(path):
    image_path = Path(path)
    assert image_path.exists() and image_path.stat().st_size > 0


@pytest.mark.skipif(
    not DRAWING_ENGINE.startswith("dall-e-") and not DRAWING_ENGINE.startswith("gpt-image-"),
    reason="Not an OpenAI model",
)
@pytest.mark.parametrize("model", OAI_MODELS)
def test_gpt_image_create(model):
    gpt_image = Interface("gpt_image")
    paths = gpt_image(
        "a fluffy cat with a cowboy hat", model=model, size=1024, response_format="url"
    )
    assert_image_file_created(paths[0])
...
def test_gpt_image_variation(model):
    img_path = Path(__file__).parent.parent.parent.parent / "artifacts" / "images" / "cat.png"
    gpt_image = Interface("gpt_image")
    paths = gpt_image(
        operation="variation",
        model=model,
...
@pytest.mark.skipif(
    DRAWING_ENGINE.startswith("dall-e-"),
    reason="For whatever reason, this test doesn't work for dall-e-2 even though I followed the documentation. Not sure what's wrong. Maybe it's a bug in the OpenAI API (testing version: openai==1.76.0",
)
