...
from symai.providers.openai.engines.responses import ResponsesEngine
...
def test_unsupported_features_fail_before_client_invocation(
    ...
    engine = ResponsesEngine(client=cast("Client", RecordingClient()), model=model)
...
def test_provider_errors_are_normalized_with_chained_cause(
    ...
    engine = ResponsesEngine(client=cast("Client", FailingClient()), model="gpt-5.4")
    request = LanguageModelRequest(
        messages=(UserMessage(content=(TextContent(text="hello"),)),),
    )
