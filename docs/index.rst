...
-  `SymbolicAI <#symbolicai>`__
   ...
   -  `😶‍🌫️ How does it work? <#️-how-does-it-work>`__

      -  `📚 Symbolic operations <#-symbolic-operations>`__
      ...
      -  `🧠 Causal Reasoning <#-causal-reasoning>`__

   -  `😷 Operations <#-operations>`__

      -  `🧪 Custom Operations <#-custom-operations>`__
   ...
   -  `🕷️ Interpretability, Testing &
      Debugging <#️-interpretability-testing--debugging>`__
...
OpenAI’s neural engines, i.e. GPT-3 Davinci-003, DALL·E 2 and Embedding
...
-  **Neuro-Symbolic Engine**: `OpenAI’s LLMs
   (GPT-3) <https://beta.openai.com/docs/introduction/overview>`__
-  **Embedding Engine**: `OpenAI’s Embedding
...
-  **[Optional] SpeechToText Engine**: `OpenAI’s
...
-  **[Optional]**\ `CLIP <https://openai.com/blog/clip/>`__\ **Engine**:
   🤗 `Hugging Face <https://huggingface.co/>`__ (experimental image and
   text embeddings)
...
-  **SpeechToText Engine**: ``ffmpeg`` for audio processing (based on
   OpenAI’s `whisper <https://openai.com/blog/whisper/>`__)
...
for the respective engine. Let’s see an example:
...
SymbolicAI’ API closely follows best practices and ideas from
...
the model’s responses based on real-world data.
...
Due to limited compute resources we currently rely on OpenAI’s GPT-3 API
...
To “read” text from images we can perform optical character recognition
...
Don’t worry, we would never hide an image of a cat with a hat from you.
...
Finally, let’s assume you want to create a entirely new engine, but
...
section `Custom Operations <#🧪-custom-operations>`__).

⚡Limitations
------------

Uff… this is a long list. We are still in the early stages of
...
-  `Newell and Simon’s Logic Theorist: Historical Background and Impact
   on Cognitive
